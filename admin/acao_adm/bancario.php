...
$filtro_conta = filter_input(INPUT_GET, 'conta');
                ...
                <div style=" align-items:center;"><i class="bi bi-clipboard"></i></div> Ação Administrativa
                    ...
                    <div  id="cadastrosMenu">
                        <ul class="btn-toggle-nav list-unstyled fw-normal pb-1 small ps-3">
                            <li class=" menu-li menu-li-atual"><a href="/admin/acao_adm/bancario.php" class="link-light text-decoration-none"><i
                                        class="bi bi-bank"></i>Bancária</a></li>
                            <li class=" menu-li"><a href="/admin/acao_adm/vendas.php" class="link-light text-decoration-none"><i
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-header-lancamento">
                    <h3>Ação Administrativa - Bancário</h3>
                </div>
            </div>
            <div class="card-header-div">
                <div class="card-header-borda">
                    <div class="tab-pane fade show active" id="vendas" role="tabpanel" aria-labelledby="vendas-tab">
                        ...
                        <form method="get" action="bancario.php">
                            <div class="d-flex flex-row w-100 justify-content-between">
                                <div class="w-100 d-flex flex-column">
                                    <div class="d-flex flex-row">
                                        ...
                                        <div style="width: calc(100% / 2.5);">
                                            <label>Conta:</label>
                                            <select name="conta" class="form-control" onchange="this.form.submit()">
                                                <option value="">Selecione</option>
                                                <?php
                                                if($filtro_empresa != null) {
                                                $contas = Ban01::read(id_empresa: $filtro_empresa);
                                                    foreach ($contas as $conta) {
                                                        echo "<option value=\"{$conta->id}\" " . ($filtro_conta == $conta->id ? "selected" : "") . ">{$conta->nome}</option>";
                                                    }
                                                }
                                                ?>
                                            </select>
            ...
            <div class="card-body">
                ...
                <div class="accordion">
                    ...
                    if($filtro_empresa != null) {
                        $contas = Ban01::read(id_empresa: $filtro_empresa);
                        foreach ($contas as $conta) {
                            $lancamentos = Ban02::read(filtro_conta: $conta->id, filtro_data_inicial: $filtro_data_inicial, filtro_data_final: $filtro_data_final);
                            if(empty($lancamentos)) {
                            //    continue;
                            }
                            if($filtro_conta != null && $filtro_conta != $conta->id) continue;
                            ?>
                            <div class="accordion-item" style="position: sticky;">
                                <h2 class="accordion-header" id="heading<?=$conta->id?>" style="position: sticky; margin-bottom: 0.1em;">
                                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?=$conta->id?>" aria-expanded="false" aria-controls="collapse<?=$conta->id?>" style="color: black;">
                                        <?=$conta->nome?>
                                    </button>
                                </h2>
                                <div id="collapse<?=$conta->id?>" class="accordion-collapse collapse" aria-labelledby="heading<?=$conta->id?>"  data-bs-parent="#accordionExample" style="position: sticky;">
                                    <div class="accordion-body " style="position: sticky; max-height: 50vh; overflow: scroll;">
                                    <?php
                                    
                                    if(count($lancamentos) == 0) {?>
                                        <p>Não há lançamentos para esta conta.</p>
                                    <?php } else { ?>
                                        ...
                                        <?php foreach ($lancamentos as $lancamento) { ?>
                                            <tr>
                                            <td><?=(new DateTime($lancamento->data))->format('d/m/Y')?></td>
                                            <td><?=$lancamento->descricao?></td>
                                            <td><?=number_format($lancamento->valor, 2, ',', '.')?></td>
                                            <td><?=($lancamento->valor > 0 ? 'Crédito' : 'Débito')?></td>
                                            </tr>
                                        <?php } ?>
                                        </tbody></table>
                                    <?php }?>
                                    ...
                                    <!-- <hr> -->
                                     ...
                                     <form method="post" action="acao_manager.php">
                                        ...
                                        <input name="conta" type="hidden" value="<?= $conta->id ?>">
