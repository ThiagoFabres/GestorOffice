...
$get_filtro_nf = filter_input(INPUT_GET, 'filtro_nf') ?? null;
    ...
    <div class="main" id="container">
        <div class="row">
            <div class="col-md-12" style="padding: 0;">
                <div class="card">
                    ...
                    <?php require_once __DIR__ . '/../componentes/card-headers/financeiro/pagar/card-header.php' ?>
                    <?php require_once __DIR__ . '/../componentes/tabelas/financeiro/pagar/tabela-pagar.php' ?>
        ...
        <div class="relatorios-botoes w-100" style="float: left;">
            <div class="d-flex flex-column">
                <button class="btn btn-primary btn-sm" id="botao-gerar-pdf" onclick="<?php if($get_pdf || $get_excel) { ?>gerarpdf('pagar', <?= json_encode($nome_empresa) ?>, 'completo');<?php } else { ?>window.location.href='<?= $caminho ?><?= empty($filtros) ? '?' : '&' ?>pdf=1';<?php } ?>">Gerar PDF Completo</button>
                <button class="btn btn-primary btn-sm" id="botao-gerar-pdf" onclick="<?php if($get_pdf || $get_excel) { ?>gerarpdf('pagar', <?= json_encode($nome_empresa) ?>, 'reduzido');<?php } else { ?>window.location.href='<?= $caminho ?><?= empty($filtros) ? '?' : '&' ?>pdf=reduzido';<?php } ?>">Gerar PDF Reduzido</button>
            </div>
            <div class="d-flex flex-column">
...
<script>
    ...
    <?php if($get_pdf || $get_pdf == 'reduzido') { ?>
        gerarpdf('pagar', <?= json_encode($nome_empresa) ?>, '<?= $get_pdf === 'reduzido' ? 'reduzido' : 'completo' ?>');
        setTimeout(() => {
