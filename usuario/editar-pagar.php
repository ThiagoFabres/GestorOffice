...
<body id="body">
    ...
    if ($get_id != null && isset($recebimento)) {
        $data_lanc = new DateTime(filter_input(INPUT_POST, 'data_lanc')) ?? new DateTime($recebimento->data_lanc);
    } else {
...
<?php if (isset($acao) && $acao == 'adicionar' || $acao == 'visualizar') { ?>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', function () {
            var modalEl = document.getElementById(<?php if ($view == 'receber') {
                if ($acao == 'adicionar') { ?> 'modal_receber' <? } else if ($acao == 'visualizar') { ?> 'modal_visualizar' <?php }
            } ?>);
