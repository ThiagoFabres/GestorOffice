    ...
    <div class="main" id="container" >
        <div class="card card-fechamento-responsivo" style=" overflow:visible !important;">
            ...
            <div class="card-header d-flex flex-row justify-content-between">
                <button class="w-50 d-flex btn btn-primary dre-menu-btn" onclick="window.location.href='parametro_receber.php'">
                    <h3>Receitas</h3>
                </button>

                <button class="w-50 d-flex btn btn-primary dre-menu-btn" style="border-bottom: 2px solid #5856d6;" onclick="window.location.href='parametro_pagar.php'">
                    <h3>Despesas</h3>
                </button>
            </div>
                    ...
                    <form action="parametro_manager.php" method="post">
                        ...
                        <div class="d-flex flex-row justify-content-evenly">
                            <div class="d-flex flex-column" style="width: 40%;">
                                ...
                                <select type="text" class="form-control" id="tipo_pagamento" name="tipo_pagamento">
                                    <option>Selecione</option>
                                    <?php foreach(TipoPagamento::read(idempresa: $_SESSION['usuario']->id_empresa) as $tipo_pagamento) { ?>
                                        <option value="<?= $tipo_pagamento->id ?>" <?= $fecha01 && $fecha01->tipo_pagamento == $tipo_pagamento->id ? 'selected' : '' ?>><?= $tipo_pagamento->nome ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="d-flex flex-column" style="width: 40%;">
                                <label>Descrição:</label>
                                <input type="text" class="form-control" name="descricao" placeholder="Descrição" value="<?= $fecha01 ? $fecha01->descricao : '' ?>">
                            </div>
                        </div>
...
<?php $erro = filter_input(INPUT_GET, 'erro'); if($erro == 'campos_obrigatorios') {
    ...
    echo "window.location.href = 'parametro_pagar.php';";
} ?>
</script>
