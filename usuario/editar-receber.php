...
if(isset($view) && $view == 'contas'){
    ...
    if(!$recebimento) header('Location: receber.php');
...
<?php $post_cadastro = filter_input(INPUT_POST, 'cadastro') ?? $post_cadastro = Cadastro::read($recebimento->id_cadastro)[0]->id_cadastro;
        ...
        $data_lanc = new DateTime(filter_input(INPUT_POST, 'data_lanc')) ?? new DateTime ($recebimento->data_lanc) ?? $data_lanc = (new DateTime())->format('Y-m-d');
...
<div class="card-header"><h3>Dados das parcelas</h3></div>
        ...
        <div class="card-header-borda">
            <div class="tab-pane fade show active" id="vendas" role="tabpanel" aria-labelledby="vendas-tab">
                ...
                <form method="post" action="editar-receber.php?view=receber&acao=editar" onkeydown="return event.key != 'Enter';">
                    <?php  if (isset($get_id)) { ?>
                        <input type="hidden" name="id" value="<?=$get_id?>">
                    <?php } ?>
                    <div class="row">

                    <div class="input-group-edit-parcela" style="width: 100%; position:relative; display:flex; flex-direction:row;">
                        ...
                        <div class="input-cadastro" style="width: 50%;">
                            ...
                            <select name="cadastro" class="form-select" id="cadastro">
                                ...
                                foreach ( $cadastros as $cadastro) { ?>
                                    <option value="<?= $cadastro->id_cadastro?>" <?php if(($post_cadastro == $cadastro->id_cadastro)) {?> selected <?php } ?> ><?= htmlspecialchars($cadastro->nom_fant, ENT_QUOTES, 'UTF-8') ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    ...
                    <div class="titulos-receber" style="display:flex; flex-direction: row;">
                        ...
                        <div class="input-subtitulo" style="width: 50%;">
                            <!--Nome: -->
                            ...
                            <select id="subtitulo" name="subtitulo" class="form-control">
                                ...
                                foreach ($todosSubtitulos as $sub) { ?>
                                    <option value="<?= $sub->id ?>" data-titulo-id="<?= $sub->id_con01 ?>"  <?php if($sub->id == $post_subtitulo){ ?> selected <?php } ?>>
                    ...
                    <div class="input-group-valores" style="display:flex; flex-direction: row;">
                        <div class="input-valor" style="width: 50%;">
                            ...
                            <input type="text" onchange="checar()" name="valor" class="form-control" placeholder="Valor" value="<?=htmlspecialchars($valor, ENT_QUOTES, 'UTF-8')?>" required>
                        </div>

                        <div class="input-parcelas" style="width: 50%;">
                            <!--Nome: -->
                            <label for="parcelas">Parcelas:</label>
                            <input type="number" onchange="checar()" name="parcelas" class="form-control" placeholder="Parcelas" value="<?=htmlspecialchars($parcelas_d, ENT_QUOTES, 'UTF-8')?>" required>
                        </div>
                    </div>
    ...
    <input type="hidden" name="data_lanc" value="<?=$data_lanc?>"
        ...
        <div class="tabelas-edit">      
            <table class="table table-striped table-bordered" style="margin-bottom: 0;">
                ...
                <tbody>
                    <tr>
                        <td><span id="totalParcelas"></span></td>
